// import { searchConcept } from '../../graphview/scripts/graphLogic.js';
	...
	owl_navbar: function(options){
		...
		this.propertyChange = function(item){
			...
			if(item.isClass){
				...
				searchConcept(item.name); // Node graph search -- passes concept over too "graphview"
			}
		};
		return this;
	},
...
*/
	owl_autocomplete : function(options){
		options = $.extend({
			...
			limit: 50, //limit size of result list to given amount
			...
			formatListItem : function(listitem, type, identifier, termarray){ //formatting of results, can be overridden
				if(termarray.length) {
					listitem.append($('<div class="name"/>').text(termarray.join(', ')).prepend($('<span/>').addClass('termlabel')));
				}
				if(type) {
					listitem.append($('<div class="type"/>').text(type));
				}
				listitem.append($('<div class="terms"/>').text(identifier).prepend($('<span/>').addClass('termlabel').text("ID: ")));
		}}, options);
		jOWL.UI.asBroadcaster(this);
		...
		results.open = function(q, cache){
			...
			if(q){
				if(!cache || results.isEmpty()) { results.cache = jOWL.query(q, options); }
					...
					var newcache = {};
					for(x in results.cache){ // Search for concepts
						var entry = results.cache[x];
						var found = false;
						var newentries = [];
						if(x.searchMatch(q) > -1) { found = true; }
						for(var i = 0;i<entry.length;i++){
							entry[i].filter = 'owl:Concept'; // SHEILA ADDED FOR FILTER
					...
					// var temp = $('ENVO_00000362').owl_propertyLens({
					// 	"term" : {split: ", "},
					// 	"sparql-dl:PropertyValue(owl:Class, ?p, ?t)": {split: ", "}});

					// console.log('sparql-dl:PropertyValue(owl:Class, ?p, ?t)'+'?p'+'?t');
					// jOWL.SPARQL_DL(query, fill);


					// samplerelation = {term: "environmental system", locale: "en", type: "owl:Class", filter:"owl:Relation" }
					// relations = [];
					// relations.push(samplerelation);
					// newcache[newcache.length] = relations;

					results.cache = newcache; // Put them all together
				}
...
*/
	owl_treeview : function(options){
		options = $.extend({
			...
			rootAxiom : false, //if true then topnode is (owl) 'Axiom'
			isStatic : true, // if static then selections will refresh the entire tree
			addChildren : true //add a given objects children to the treeview as well
		}, options);
		...
		function TreeModel(owlobject){
			...
			if(options.rootAxiom) { traverse(h, tree.root(jOWL("Axiom"))); }
			else {
		...
		function Tree(node, treemodel, options){
			...
			fn.node = function(text, isRoot){ //creates a new node
				...
				function toggle(){ //open and expands parent node
				...
				function toNode(){ //displays the children (style)
...
/** Uses templating
options:
onChange: owl:Class, owl:Axiom, etc..., tell the widget what to do with the different kinds of Ontology Objects.
		...
		owl_propertyLens : function(options){
			...
			this.link = function(source, target, htmlel, fn){
				...
				backlink.show().unbind('click').click(function(){
					self.broadcast(source); self.propertyChange(source);
					backlink.hide();
				});
			...
			var action = {
				...
				"rdfs:subClassOf": function(item){
					if(!(item.isClass)) { return; }
					return $.map(
							jOWL.Xpath('*', item.jnode)
								.filter(function(){return this.nodeName == "rdfs:subClassOf"; }),
							function(n, i){ return {"rdfs:subClassOf": jOWL($(n).RDF_Resource())};
							});
				},
	...
	owl_datafield: function(options){
		...
		this.serialize = function(){
			...
			$('.'+options.selectorClass, self).each(function(){
				...
				if( $this.is("select")){
						var s = $this.get(0);
						var axiom = $(s[s.selectedIndex]).attr('title');
						q.PropertyValue.push([prop, axiom]);
					}
				else {
