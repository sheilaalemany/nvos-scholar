...
oReq.open("GET", "../treeview/data/envo.owl");
oReq.send();
...
jOWL.Ontology.Axiom = function(jnode){
	this.parse(jnode);
};

jOWL.Ontology.Axiom.prototype = {
	jOWL : jOWL.version,
	equals : function(id){
	...
	description : function(){
		return $.map(this.annotations(__.obo('IAO_0000115') || __.rdfs('comment')), function(n){ return $(n).text();});
	},
	/**
	...
	*/
	...
	label : function(){
		...
		if(jOWL.options.niceClassLabels && (this.isClass || this.isAxiom)){
...
jOWL.Ontology.prototype = jOWL.Ontology.Axiom.prototype;
...
jOWL.Ontology.Individual = function(jnode, owlclass){
	...
	this.type = __.owl("Axiom");
	if(owlclass){ this.owlClass(owlclass);}
};

jOWL.Ontology.Individual.prototype = $.extend({}, jOWL.Ontology.Axiom.prototype, {
	isAxiom : true,
	...
	sourceof : function(property, target, options){
		options = $.extend({
			...
			transitive : true,
			ignoreGenerics : false, //if a parent has an identical property, with another target 'Axiom', skip that restriction
			ignoreClasses : true,
			valuesOnly : true
		}, options);
...
jOWL.Ontology.Restriction.prototype = {
	...
	getTarget : function(){
		if(!this.target){ return jOWL('Axiom');}
...
jOWL.Ontology.Property.prototype = $.extend({}, jOWL.Ontology.Axiom.prototype,{
	isProperty : true,
...
jOWL.Ontology.DatatypeProperty.prototype = $.extend({}, jOWL.Ontology.Axiom.prototype, jOWL.Ontology.Property.prototype, {
...
jOWL.Ontology.ObjectProperty.prototype = $.extend({}, jOWL.Ontology.Axiom.prototype, jOWL.Ontology.Property.prototype, {
...
jOWL.Ontology.Class.prototype = $.extend({}, jOWL.Ontology.Axiom.prototype, {
	isClass : true,
	...
	parents : function(){
		...
		if(!oParents.length){ oParents.push(jOWL('Axiom'));}
		else if(oParents.length > 1){ oParents.filter(function(){return this.name != ('Axiom');});} //Remove Axiom reference if other parents exist
...
@return a jOWL.Ontology.Array containing top nodes (classes directly subsumed by 'owl:Axiom')
...
*/
	hierarchy : function(addInverse){
		...
		function URIARR(p_arr, obj){
			...
			if(p_arr.each){
				p_arr.each(function(){
					...
					if(this.URI == __.owl()+'Axiom'){ return;}
		...
		function traverse(concept){
			...
			if(parents.length == 1 && parents.contains(__.owl()+'Axiom')){ endNodes.pushUnique(concept); return;}
			else
			{
					...
					results.filter(function(){
						...
						if(restr.property.URI == parentsource.property.URI){
							...
							if(!options.ignoreGenerics){
								...
								if(ptarget.isAxiom){
									keep = restr.getTarget().isAxiom && parentsource.target != restr.target;
		...
		case "ID":
			if(i.IDs === null || wipe){
				...
				var rAbout = jOWL.Xpath("/"+__.rdf("RDF")+"/*[@"+__.rdf("about")+"]").each(function(){
					...
					var jowl = jOWL.getResource($(this));
					if(!jowl){ return;}
						if(jowl.isClass || jowl.isProperty || jowl.isAxiom){
...
jOWL.parse = function(doc, options){
	...
	jOWL.Axiom = new jOWL.Ontology.Axiom($(jOWL.create(__.owl, "Class").attr(__.rdf, 'about', __.owl()+'Axiom').node));
	jOWL.Axiom.type = false;
	return this;
};
...
jOWL.getResource = function(resource, options){
	if(!jOWL.document){ throw "You must successfully load an ontology before you can find anyAxiom";}
	if(!resource){ throw "No resource specified";}
	var node;
	var opts = $.extend({}, options);
	if(typeof resource == 'string'){
		resource = jOWL.resolveURI(resource);
		if(resource == 'Axiom' || resource == __.owl()+'Axiom'){ return jOWL.Axiom;}
		...
		if(!match){
			if(jOWL.isExternal(resource)){
				console.log("undeclared resource: "+resource);
				return new jOWL.Ontology.Axiom(resource);
			}
	...
	switch(xmlNode.nodeName){
		...
		default :
			switch(xmlNode.namespaceURI){
				case __.owl(): if(xmlNode.nodeName == __.owl("Axiom") ){ return jOWL.Ontology.Individual;} return false;
...
options:
    filter: filter on a specific type, possible values: Class, Axiom, ObjectProperty, DatatypeProperty
...
options.modify(item) Thing to do with each item of the array
...
jOWL.SPARQL_DL.prototype = {
	...
	*/
	...
	process: function(entry, resultobj, options){
		...
		var q = entry[0];
		var sizes = {
			"Type": [__.owl('Axiom'), __.owl('Class')],
			"DirectType": [__.owl('Axiom'), __.owl('Class')],
	...
	fn : {
			...
			function traverse(node, match){
				...
				if(atom.source.isURI() && atom.target.isURI()){//assert
					...
					var classlist = atom.target.getURIs(),
						...
						for(x in classes){
							var individuals = jOWL.index("Axiom")[x];
							if(individuals){
			...
			"Axiom" : function(syntax, resultobj, options){
				jOWL.SPARQL_DL.priv.IDQuery(syntax[0], "isAxiom", resultobj, options);
			},
			...
			"PropertyValue" : function(syntax, resultobj, options){
				...
				if(!atom.source.getURIs()){
					jOWL.SPARQL_DL.priv.IDQuery(atom.source.value, ["isClass", "isAxiom"], resultobj, options);
					return;
				}
