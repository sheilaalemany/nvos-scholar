  ...
  User.findOne({ _id: req._id }, (err, user) => {
    if (!user) {
      ...
      return res.status(200).json({
        status: true,
        user: _.pick(user, ["fullName", "email", "articles", "search", "click","feedback"])
      });
    }
  });
};
...
/**
 * Gets called from the index.router.js
 * Saves a feedback article to the users profile
 */
module.exports.saveFeedback = (req, res, next) => {
  User.findOneAndUpdate(
    { _id: req._id },
    {
      $addToSet: { feedback: req.body.feedback }
    },
    {
      new: true
    },
    function(err, savedFeedback) {
      if (err) {
        res.send("Error saving feedback history");
      } else {
